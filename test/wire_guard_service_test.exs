...
defmodule WireGuardServiceTest do
  ...
  setup do
    ...
    if enabled do
      ...
      # Stop existing service so supervisor restarts it with test config (listen_port etc.)
  ...
  @tag :requires_wireguard
  test "add_peer with invalid public key size", _context do
    # Service may already be running (e.g. started by application when WIREGUARD_ENABLED=1)
    case WireGuardService.start_link([]) do
      {:ok, _pid} -> :ok
