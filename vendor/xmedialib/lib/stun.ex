...
### ----------------------------------------------------------------------
###
### Diode vendor note: MESSAGE-INTEGRITY uses :crypto.mac/4 instead of
### removed :crypto.hmac/3 (OTP 28+). Upstream: Hex xmedialib 0.1.2.
### See docs/vendoring-xturn.md
###
### ----------------------------------------------------------------------
...
defmodule XMediaLib.Stun do
  ...
  def process_stun(stun_binary, key, fingerprint, integrity) do
    case stun_binary do
      <<@stun_marker::2, m0::5, c0::1, m1::3, c1::1, m2::4, length::16,
        @stun_magic_cookie::32, transactionid::96, rest3::binary>> ->
      ...
      _ ->
        Logger.debug("Not a STUN message: #{Base.encode16(stun_binary)}")
        {:error, :not_stun}
    end
  end
  ...
  defp decode_attr_xaddr(<<0::size(8), 2::size(8), xport::size(16), xaddr::size(128)>>, tid) do
    ...
    <<i0::size(16), i1::size(16), i2::size(16), i3::size(16), i4::size(16), i5::size(16),
      ...
      <<bxor(xaddr, bor(bsl(@stun_magic_cookie, 96), tid))::size(128)>>
  ...
  defp do_check_integrity(stun_binary, integrity, message, key) do
    with ^integrity <- :crypto.mac(:hmac, :sha, key, message),
         <<h::size(16), old_size::size(16), payload::binary>> <- message,
  ...
  defp insert_integrity(stun_binary, key) do
    ...
    integrity = :crypto.mac(:hmac, :sha, key, <<h::size(16), s::size(16), message::binary>>)
